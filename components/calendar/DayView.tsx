            ...
            <div className="relative h-[1200px] w-full overflow-y-auto custom-scrollbar">
                ...
                {layoutEvents.map(event => {
                    ...
                    return (
                        ...
                        >
                            <div
                                className="absolute top-0 left-0 right-0 h-1.5 cursor-ns-resize z-30 flex items-center justify-center hover:bg-black/5 transition-colors"
                                onPointerDown={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    setResizing({
                            ...
                            >
                                <h4 className="font-bold text-[10px] lg:text-xs truncate leading-none cursor-pointer hover:opacity-75 shrink-0" style={{ color: getEventStyle(event).color, filter: 'brightness(0.7)' }}>{event.title}</h4>
                                {duration >= 45 && totalColumns === 1 && event.description && (
                                    <p className="text-[10px] text-gray-500 line-clamp-1 opacity-70 leading-none mt-0.5">{event.description}</p>
                                )}
                            </div>

                            <div
                                className="absolute bottom-0 left-0 right-0 h-1.5 cursor-ns-resize z-30 flex items-center justify-center hover:bg-black/5 transition-colors"
                                onPointerDown={(e) => {
                                    e.preventDefault();
                                    e.stopPropagation();
                                    setResizing({
