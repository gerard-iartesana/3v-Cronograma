                ...
                {months.map(monthIdx => {
                    ...
                    return (
                        <div key={monthIdx} className="bg-white border border-gray-200 p-4 rounded-[2rem] flex flex-col h-full min-h-[200px] transition-all hover:border-gray-300 hover:bg-gray-50 group/month shadow-md">
                            <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-800 mb-4 text-center border-b border-gray-100 pb-2 group-hover/month:text-black transition-colors">
                ...
                {hoveredDay && tooltipPos && (
                    ...
                    >
                        <div className="text-[11px] font-black text-gray-500 uppercase tracking-[0.25em] mb-4 border-b border-gray-100 pb-3 flex justify-between items-center">
                            ...
                            <span className="text-[#dc0014] bg-gray-100 px-2.5 py-1 rounded-full">{formatDuration(hoveredDay.events.reduce((acc, e) => acc + parseDurationToMinutes(e.duration), 0))}</span>
                        </div>
                        <div className="space-y-4">
                            {hoveredDay.events.slice(0, 6).map(e => (
                                <div key={e.id} className="text-[13px] font-bold text-gray-800 flex items-start justify-between gap-3">
                                    ...
                                    <div className="flex gap-1 flex-shrink-0 mt-0.5">
                                        {e.tags.filter(t => tagColors?.[t]).map(t => (
                                            <span
                                                key={t}
                                                className="text-[7px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded-full border border-gray-200"
                                                style={{
                                                    color: tagColors![t],
